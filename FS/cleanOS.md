...
Также стоит обновить загрузчик GRUB, путем пересоздания его конфигурации
```bash
update-grub
```
Поскольку мы не трогали текущее ядро и не меняли конфигурацию системы, а только вычистили мусор от остатков удаленных ядер, то команда ниже избыточна (пересоздание initramfs для текущего ядра)
```bash
update-initramfs -u
```
